...
from split_model import DistMidiBert, DistMidiBertFFN, DistMidiBertFFNDecomp, DistMidiBertQuant, DistMidiBertFFNQuant, DistMidiBertFFNQuantRes, load_split_model_from_checkpoint, load_split_model_from_checkpoint_FFN, load_split_model_from_checkpoint_FFN_Decomp
...
def main():
    ...
    # Initialize RPC with device mapping
    rpc_backend_options = rpc.TensorPipeRpcBackendOptions()
    rpc_backend_options.set_device_map("cloud", {
        "cuda:0": "cuda:0"
    })
    ...
    dict_path = '/import/c4dm-05/bja01/models/splitfinetuning/MidiBERT/data/dict/CP.pkl'
    print("\nLoading dictionary...")
    ...
    checkpoint_path = '/import/c4dm-05/bja01/models/splitfinetuning/MidiBERT/pretrain/pretrain_model.ckpt'
    split_layer = args.split_layer
    
    print("\nLoading split models from checkpoint...")
    if model_choice == 1:
        print("Using Split Learning (Original) model")
        front_model, back_model = load_split_model_from_checkpoint(
            checkpoint_path, bertConfig, e2w, w2e, split_layer
        )
        ...
        print("Using Split Fine-tuning model")
        front_model, back_model = load_split_model_from_checkpoint_FFN(
            checkpoint_path, bertConfig, e2w, w2e, split_layer
        )
        classifier = DistMidiBertFFN(
            devices=devices,
            bertConfig=bertConfig,
            e2w=e2w,
            w2e=w2e,
        ...
        print("Using Split Fine-tuning with Decomposition model")
        front_model, back_model = load_split_model_from_checkpoint_FFN_Decomp(
            checkpoint_path, bertConfig, e2w, w2e, split_layer
        )
        classifier = DistMidiBertFFNDecomp(
            devices=devices,
            bertConfig=bertConfig,
            e2w=e2w,
            w2e=w2e,
        ...
        print("Using Split Fine-tuning with Quantisation model")
        front_model, back_model = load_split_model_from_checkpoint(
            checkpoint_path, bertConfig, e2w, w2e, split_layer
        )
        classifier = DistMidiBertQuant(
            devices=devices,
            bertConfig=bertConfig,
            e2w=e2w,
            w2e=w2e,
        ...
        print("Using Split Fine-tuning (FFN) with Quantisation model")
        front_model, back_model = load_split_model_from_checkpoint(
            checkpoint_path, bertConfig, e2w, w2e, split_layer
        )
        classifier = DistMidiBertFFNQuant(
            devices=devices,
            bertConfig=bertConfig,
            e2w=e2w,
            w2e=w2e,
        ...
        print("Using Split Fine-tuning (FFN) with Quantisation and Residual model")
        front_model, back_model = load_split_model_from_checkpoint_FFN(
            checkpoint_path, bertConfig, e2w, w2e, split_layer
        )
        classifier = DistMidiBertFFNQuantRes(
            devices=devices,
            bertConfig=bertConfig,
            e2w=e2w,
            w2e=w2e,
    ...
    class Args:
        def __init__(self):
            ...
            self.batch_size = 12
            self.num_workers = 5
            self.data_root = '/import/c4dm-05/bja01/models/splitfinetuning/MidiBERT/data/dict/CP_data'
    ...
    save_dir = os.path.join('/import/c4dm-05/bja01/models/splitfinetuning/MidiBERT/results', f"{args.task}_{args.name}")
